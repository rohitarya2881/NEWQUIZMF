...
async function displayFlashcards(quizId) {
    // Ensure latest folder structure is loaded so isBookmarked works correctly
    await loadFolderStructure();
...
function showBackupOptions() {
    ...
    m.innerHTML = `<div class="modal-content glass-card" style="max-width:480px;">
        ...
        <div class="backup-options-grid">
            ...
            <div class="backup-option" onclick="shareAllFolders();this.closest('.modal').remove()" style="border-color:#27ae60;">
                <i class="fas fa-share-alt" style="color:#27ae60;"></i>
                <h4 style="color:#27ae60;">Share All</h4>
                <p>Share full folder tree with friend</p>
            </div>
            <div class="backup-option" onclick="shareCurrentFolder();this.closest('.modal').remove()" style="border-color:#4a6fa5;">
                <i class="fas fa-share" style="color:#4a6fa5;"></i>
                <h4 style="color:#4a6fa5;">Share Folder</h4>
                <p>Share current folder + quizzes</p>
            </div>
        </div>
    </div>`;
    document.body.appendChild(m);
...
async function shareAllFolders() {
    ...
    const payload = {
        ...
        type:     'qmp_share',
        itemType: 'full_tree',
        name:     'Full Quiz Collection',
        sharedAt: new Date().toISOString(),
        folderCount:   folders.length,
        quizCount:     quizzes.length,
        questionCount: totalQ,
        // Full structure so folder tree is preserved
        structure: folderStructure,
        items:     allItems
    };

    await _uploadAndShowShareModal('Full Quiz Collection', 'full_tree', payload);
}

// ── Share current folder (with subfolders + quizzes) ──
async function shareCurrentFolder() {
    if (!currentFolder || currentFolder.id === 'root') {
        showToast('Go inside a folder first', 'warning'); return;
    }

    // Collect all items under current folder recursively
    const items = [];
    const walk  = (node) => {
        items.push(node);
        (node.children || []).forEach(walk);
    };
    currentFolder.children?.forEach(walk);
    ...
    const payload = {
        version:  '1.0',
        type:     'qmp_share',
        itemType: 'folder_tree',
        name:     currentFolder.name,
        sharedAt: new Date().toISOString(),
        quizCount:     quizzes.length,
        questionCount: totalQ,
        // Preserve full folder structure from this folder down
        folder: currentFolder,
        items:  items
    };

    await _uploadAndShowShareModal(currentFolder.name, 'folder_tree', payload);
}

// ── Common upload + show modal ────────────────
async function _uploadAndShowShareModal(name, type, payload) {
    const JSONBIN_KEY  = '$2a$10$JaUwsK9x3kbNvhFmOHXcDenDiyQYNb5wQ7VJjj0ubekAhbtmkv5ke';
    const JSONBIN_BASE = 'https://api.jsonbin.io/v3/b';
    const SHARE_BASE   = 'https://rohitarya2881.github.io/NEWQUIZMF/';
    ...
    try {
        const resp = await fetch(JSONBIN_BASE, {
            ...
            headers: {
                ...
                'X-Bin-Name':    `qmp_${name.substring(0,30)}`,
                'X-Bin-Private': 'false'
            },
        ...
        const shareUrl = `${SHARE_BASE}?import=${binId}`;
        const qCount   = payload.questionCount || 0;
        const extra    = payload.folderCount   ? ` · ${payload.folderCount} folders · ${payload.quizCount} quizzes` :
                         payload.quizCount     ? ` · ${payload.quizCount} quizzes` : '';

        // Show share modal
        document.getElementById('shareModal')?.remove();
        const encodedUrl  = encodeURIComponent(shareUrl);
        const encodedName = encodeURIComponent(name);
        const m = document.createElement('div'); m.className = 'modal'; m.id = 'shareModal';
        m.innerHTML = `<div class="modal-content" style="max-width:480px;">
            ...
            <div style="background:rgba(39,174,96,0.08);border:1px solid rgba(39,174,96,0.2);
                ...
                <div style="font-size:0.78rem;color:#aaa;">${qCount} questions${extra}</div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;">
                ...
                <button onclick="window.open('https://wa.me/?text=${encodedName}%0A${encodedUrl}','_blank')"
                    style="background:#25D366;color:white;border:none;border-radius:4px;padding:8px;cursor:pointer;font-family:inherit;font-weight:600;">
                    <i class="fab fa-whatsapp"></i> WhatsApp
                </button>
                <button onclick="window.open('https://t.me/share/url?url=${encodedUrl}&text=${encodedName}','_blank')"
                    style="background:#0088cc;color:white;border:none;border-radius:4px;padding:8px;cursor:pointer;font-family:inherit;font-weight:600;">
                    <i class="fab fa-telegram"></i> Telegram
                </button>
                <button class="secondary-btn" onclick="window.open('${escHtml(shareUrl)}','_blank')" style="justify-content:center;">
                    🔗 Open Link
                </button>
            </div>
            <div style="font-size:0.72rem;color:#aaa;text-align:center;">
                Friend opens link → sees folder structure → picks where to import → done
            </div>
        </div>`;
        document.body.appendChild(m);
